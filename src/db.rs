...
impl Db {
    ...
    pub fn active_agenda(&self) -> Result<Vec<AgendaItem>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name FROM agenda_items WHERE completed_at IS NULL ORDER BY id ASC",
        )?;
        let rows = stmt
            .query_map([], |row| {
                Ok(AgendaItem {
    ...
    pub fn todays_tasks(&self) -> Result<Vec<Task>> {
    ...
    pub fn add_task(&self, name: &str, hours: i32, minutes: i32) -> Result<()> {
        let now = Local::now().to_rfc3339();
        self.conn.execute(
            "INSERT INTO tasks (name, hours, minutes, logged_at) VALUES (?1, ?2, ?3, ?4)",
            params![name, hours, minutes, now],
        )?;
        Ok(())
    }
    ...
    pub fn metrics(&self) -> Result<Metrics> {
        ...
        let week_minutes = self.sum_minutes(week_start, week_end)?;
        ...
        Ok(Metrics {
            ...
            week_task_count: self.count_tasks(week_start, week_end)?,
            today_minutes,
            week_minutes,
        })
    }
